...
var Store = class _Store {
  ...
  gitPath;
  constructor(root2, gitDir, gitPath) {
    this.root = root2;
    this.gitDir = gitDir;
  ...
  static forProject(name) {
    const root2 = join(projectsHome(), name);
    return new _Store(root2, root2, ".");
  }
  static listProjects() {
...
function project(claims2, meta, o, level) {
  ...
  const milestone = by(claims2, ["milestone"], /* @__PURE__ */ new Set(["open"]))[0];
  const next = by(claims2, ["next_action"], /* @__PURE__ */ new Set(["open"]))[0];
  ...
  const decisions = by(claims2, ["decision", "architecture"], /* @__PURE__ */ new Set(["accepted", "active"]));
  if (decisions.length) {
  ...
  const cons = by(claims2, ["constraint"], /* @__PURE__ */ new Set(["accepted", "active"]));
  if (cons.length) {
  ...
  const opens = by(claims2, ["question", "risk"], /* @__PURE__ */ new Set(["open"]));
...
function renderResumeContext(claims2, meta = {}, opts = {}) {
  const o = {
  ...
  for (let level = 0; level <= 3; level++) {
    out = project(claims2, meta, o, level);
