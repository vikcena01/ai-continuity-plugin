...
function saveMsg(s, msg) {
  commit(s.gitDir, s.gitPath, msg);
}
function resolveOne(s, query) {
...
switch (cmd) {
  ...
  case "record-constraint": {
    ...
    const type = cmd === "record-constraint" ? "constraint" : "decision";
    const s = getStore();
    const c = s.record({ type, title, body: flag("body"), confidence: flag("confidence") ?? "confirmed" });
  ...
  case "resolve": {
    ...
    const action = bool("accept") ? "accept" : bool("reject") ? "reject" : bool("close") ? "close" : void 0;
    const s = getStore();
    let out;
    try {
