...
FIXED in 09f1a88: .claude/settings.json registers both hooks against the repo's own committed bundles via $CLAUDE_PROJECT_DIR, so a plain clone self-activates. Both hooks de-duplicate per session (src/core/once.ts) so a developer who also has the plugin installed is not double-fired.
