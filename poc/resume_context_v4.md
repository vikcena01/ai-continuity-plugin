...
## ⚠️ DEPENDENCY IMPACTS (a foundation changed — RE-EVALUATE, do not just build on these)
- [collision-retry] "Unique constraint + retry on collision" rests on [postgres-primary-store] "PostgreSQL is the source of truth", which was just superseded by [distributed-primary-store] "Move primary link store to DynamoDB global tables". Re-evaluate before acting on it.
- [analytics-clickhouse-store] "Analytics goes into a separate ClickHouse store; Postgres serves links only" rests on [postgres-primary-store] "PostgreSQL is the source of truth", which was just superseded by [distributed-primary-store] "Move primary link store to DynamoDB global tables". Re-evaluate before acting on it.
...
- [distributed-primary-store] Move primary link store to DynamoDB global tables — New requirement: sub-50ms redirects worldwide. Move the primary source of truth for links from single-region Postgres to DynamoDB global tables for multi-region low-latency reads.  _(confidence: confirmed)_
    ↳ replaced [postgres-primary-store] "PostgreSQL is the source of truth" — because: New hard requirement for global sub-50ms redirects made single-region Postgres insufficient as the primary store; moved to DynamoDB global tables.
